...
All test code lives inside main() so that multiprocessing's spawn method
(used on Windows) does not re-run tests when it reimports __main__ to
bootstrap GPU worker processes.
"""
...
def fresh_gpu():
    """Like fresh(), but also starts GPU core processes so GPROC jobs are drained."""
...
def main():
    ...
    def jump_taken(opname, flags_value):
        ...
        target = len(jcc_words) + len(inc_words)
    ...
    inc_words = encode_instruction("000+--", [0])
    halt_words = encode_instruction("000000", [])
    jmpr_words = encode_instruction("00+---", [Immediate(len(inc_words))])
    addr = 0
    ...
    c = fresh()
    callr_words = encode_instruction("00+--+", [Immediate(0)])
    halt_words = encode_instruction("000000", [])
    inc_words = encode_instruction("000+--", [0])
    ret_words = encode_instruction("000-+-", [])
    next_pc = len(callr_words)
